...
('PX001','2019-11-24','NV01','S0003','2','KH113','Da xu ly'),
('PX002','2019-12-10','NV04','S0001','3','KH112','Chua xu ly'),
('PX003','2019-08-10','NV04','S0002','1','KH114','Da xu ly'),
('PX004','2019-07-11','NV01','S0003','2','KH113','Chua xu ly');
...
create proc suataikhoan(@ten varchar(8) ,@matkhau varchar(8))
as
   update account 
          set matkhau=@matkhau where taikhoan=@ten
 go
...
-----thêm phiếu nhập
create proc themphieunhap(@maphieunhap varchar(8),@ngaynhap date,@mathukho varchar(8),@mahang varchar(8),
	@soluong int,@mancc varchar(8))
	as
		begin
	...
	go
	--sửa phiếu nhập
create proc suaphieunhap(@maphieunhap varchar(8), @ngaynhap date,@mathukho varchar(8),@mahang varchar(8),
	@soluong int,@mancc varchar(8))
	as
		begin
			IF	@maphieunhap = '' THROW 50001, 'Mã phiếu nhập không được để trống!', 1;
			...
			manhanvien=@mathukho,masach= @mahang, soluong= @soluong, mancc= @mancc  where maphieunhap=@maphieunhap
		end
	go
...
create proc suaphieuxuat(@maphieuxuat varchar(8),@ngayxuat date,@mathukho varchar(8),@mahang varchar(8),
	...
	as
		begin
			...
			IF	@ngayxuat ='' THROW 50001, 'Ngày không được để trống!', 1;
			IF	@trangthai ='' THROW 50001, 'Trạng thái không được để trống!', 1;
			update phieuxuat set ngayxuat= @ngayxuat,
			manhanvien=@mathukho,soluong=@soluong,makhachhang=@makhachhang,trangthai=@trangthai where maphieuxuat=@maphieuxuat
		end
	go
