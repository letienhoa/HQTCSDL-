...
create table sach
	( tensach   varchar(20) ,   
	 ...
	 tenloaisach	varchar(8),
	 soluong     int,
	 make  varchar (8) not null
	constraint pk_sach primary key (masach),
constraint ck_sach unique (tenloaisach,make)
	);
...
constraint ck_phieunhap unique (manhanvien,mancc,masach)
	);
    ...
    constraint ck_phieuxuat unique (manhanvien,masach,makhachhang)
		);
go
---------------------------
--nhap gia tri
...
go
alter table sach
 with nocheck add constraint fk_hoanghoa_ke
...
go
alter table sach
check constraint fk_hoanghoa_ke;
	go	
...
End;
go
create function test(@x varchar)
    returns int
	begin
	select 
...
IF	@trangthai ='' THROW 50001, 'Trạng thái không được để trống!', 1;
   ...
   --NHÀ CUNG CẤP
	...
	go
	--kieemr tra truoc khi them vao
	create proc kiemtrancc(@mancc varchar(8))
	as
	select * from nhacungcap where mancc=@mancc
	go
   -- thêm nhà cung cấp
   create proc themncc(@mancc varchar(8),@tenncc varchar(20),@diachincc varchar(20),
   @sotk varchar(12))
   as
	insert into nhacungcap values(@mancc,@tenncc,@diachincc,@sotk)
	go
	--sửa nhà cung cấp
	create proc suancc @mancc varchar(8),@tenncc varchar(20),@diachincc varchar(20),
   @sotk varchar(12)
   as
   begin
 ...
 create proc kiemtrakhachhang(@makhachhang varchar(8))
	...
	go  
	-- thêm vào 
	 create proc themkhachang(@makhachhang varchar(8),@tenkhachhang varchar(20),@sex varchar(10),
       @diachi  varchar(20))
   as
	insert into khachhang values(@makhachhang,@tenkhachhang,@sex,@diachi)
	go 
	-- chinh sua khachhang
	 create proc suakhachhang @makhachhang varchar(8),@tenkhachhang varchar(20),@sex varchar(10),
       @diachi  varchar(20)
   as
   begin
